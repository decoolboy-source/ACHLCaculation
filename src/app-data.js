   ...
   pressureGroup:'neutral',standardModule:null,hasLocalExhaust:false,
    ...
    i18n: {
      ...
      en: {
        ...
        'Vận tốc thiết kế phải > 0':'Design velocity must be > 0', 'Không xác định':'Undetermined',
        'Khớp đúng mức chuẩn':'Exact standard match',
        'Ngoài dải bảng (dùng mức nhỏ nhất)':'Outside table range (using the smallest level)',
        'Ngoài dải bảng (dùng mức lớn nhất)':'Outside table range (using the largest level)',
        '⚙ Nội suy {lo}-{hi} kW':'⚙ Interpolated {lo}-{hi} kW',
        'Quạt thông gió (không cần coil)':'Ventilation fan (no coil needed)',
        'ΔT Data Center phải > 0':'Data Center ΔT must be > 0',
        'Roof type không tồn tại: {rt}. Dùng R1-R6':'Roof type does not exist: {rt}. Use R1-R6',
        'Lớp {n}: không tìm thấy vật liệu "{id}"':'Layer {n}: material "{id}" not found',
        '{name}: độ dày = 0 → bỏ qua':'{name}: thickness = 0 → skipped',
        '{name}: không có λ':'{name}: no λ value',
        'U = {u} W/m²K — rất cao, kiểm tra lại cấu tạo':'U = {u} W/m²K — very high, check the construction',
        'U = {u} W/m²K — rất thấp, kiểm tra độ dày cách nhiệt':'U = {u} W/m²K — very low, check the insulation thickness',
        'Vách ngăn #{n}':'Partition #{n}',
        '{label}: chưa nhập diện tích (S=0) — bỏ qua, không tính vào tổng tải':'{label}: area not entered (S=0) — skipped, not counted in the total load',
        '{label}: chưa chọn loại vách/nhập U — đang dùng mặc định U=0.45 (gạch 200mm), kiểm tra lại nếu vách thực tế khác':'{label}: no wall type/U entered — using default U=0.45 (200mm brick), double-check if the actual wall differs',
        '{label}: phòng kề đã chọn không còn tồn tại — kiểm tra lại':'{label}: the selected adjacent room no longer exists — please check',
        ...
        '* THÔNG SỐ CẦN KIỂM TRA (vượt ngưỡng tiêu chuẩn)':'* PARAMETERS TO REVIEW (exceed standard threshold)',
        'Mức độ':'Severity', 'Thông số':'Parameter', 'Chi tiết':'Detail', 'Tiêu chuẩn tham chiếu':'Reference standard',
        'I. TIÊU CHUẨN ÁP DỤNG':'I. APPLICABLE STANDARDS', 'II. PHƯƠNG PHÁP TÍNH TOÁN':'II. CALCULATION METHOD',
        'III. THÔNG SỐ THAM KHẢO THỰC TẾ (không có giá trị pháp lý)':'III. PRACTICAL REFERENCE VALUES (not legally binding)',
        'FCU/AHU catalog — dải công suất tham khảo thị trường VN 2024, xác nhận với nhà sản xuất':'FCU/AHU catalog — capacity range references the VN 2024 market, confirm with the manufacturer',
        'Đơn giá BOM (VND) — tham khảo thị trường 2024, sai số ±30-50%':'BOM unit prices (VND) — reference the 2024 market, ±30-50% margin of error',
        'Hệ số ζ phụ kiện ống gió — giá trị trung bình SMACNA, tra đồ thị để chính xác':'Duct fitting ζ coefficient — SMACNA average value, consult the chart for accuracy',
        'Mật độ tải IT kW/m² DC Rack — ước tính, đo thực tế để thiết kế chính xác':'IT load density kW/m² DC Rack — estimated, measure actual values for accurate design',
        'III. LƯU Ý PHÁP LÝ':'III. LEGAL NOTES',
        'Thông số tham khảo thực tế (FCU catalog, đơn giá VND) không có giá trị pháp lý.':'Practical reference values (FCU catalog, VND pricing) are not legally binding.',
        'Kỹ sư phải xác nhận với catalog nhà sản xuất trước khi chọn thiết bị.':'The engineer must confirm with the manufacturer catalog before selecting equipment.',
        'Số liệu khí hậu: QCVN 02:2022/BXD. Một số tỉnh dùng trạm gần nhất.':'Climate data: QCVN 02:2022/BXD. Some provinces use the nearest station.',
        'MultiHVAC Calculator — Báo cáo tính toán phụ tải nhiệt':'MultiHVAC Calculator — Heat Load Calculation Report',
        'Ngày thực hiện':'Date performed', 'KPI Tổng hợp':'Summary KPIs', 'Giá trị':'Value', 'Đơn vị':'Unit',
        'Diện tích sàn tổng':'Total floor area', '% Gió tươi / Cấp':'% Fresh / Supply', 'W/m² sàn':'W/m² floor',
        'Dài(m)':'Length(m)', 'Rộng(m)':'Width(m)', 'Cao(m)':'Height(m)', 'Thể tích(m³)':'Volume(m³)',
        'T cấp(°C)':'Supply T(°C)', 'L_hồi(m³/h)':'L_return(m³/h)',
        'Q_vách(kW)':'Q_wall(kW)', 'Q_mái(kW)':'Q_roof(kW)', 'Q_sàn(kW)':'Q_floor(kW)', 'Q_vách_ngăn(kW)':'Q_partition(kW)',
        'Q_kính(kW)':'Q_glass(kW)', 'Q_người_s(kW)':'Q_people_s(kW)', 'Q_đèn(kW)':'Q_lighting(kW)',
        'Q_hiện_tổng(kW)':'Q_sensible_total(kW)', 'Q_ẩn(kW)':'Q_latent(kW)',
        'Không cần coil':'No coil needed',
        'Catalog tham chiếu thị trường VN 2024. Xác nhận với nhà sản xuất trước khi chọn thiết bị.':'Catalog references the VN 2024 market. Confirm with the manufacturer before selecting equipment.',
        'Loại phòng':'Room type', 'Phương pháp gió':'Airflow method', 'Hiệu suất sử dụng(%)':'Utilization(%)',
        'Lưu lượng(m³/h)':'Airflow(m³/h)',
        'Khối lượng':'Quantity', 'Đơn giá (VND)':'Unit price (VND)', 'Thành tiền (VND)':'Amount (VND)',
        'TỔNG CỘNG (VND):':'TOTAL (VND):', 'TỔNG CỘNG (triệu VND):':'TOTAL (million VND):', 'GHI CHÚ':'NOTE',
